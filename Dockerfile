...
RUN apt-get update
RUN wget http://ftp.gnu.org/gnu/glibc/glibc-2.25.tar.gz ; tar zxf glibc-2.25.tar.gz ; cd glibc-2.25/ ; mkdir build ; cd build/ ; ../configure --prefix=/usr ; make -j4 ; make install
